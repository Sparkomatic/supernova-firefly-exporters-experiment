...
Pulsar.export(async (sdk: Supernova, context: PulsarContext): Promise<Array<AnyOutputFile>> => {
  ...
  // Filter tokens to only include those from specified top-level groups
  const allowedTopLevelGroups = ['primitive', 'semantic']
  
  // Add components to the allowed groups if the configuration option is enabled
  if (exportConfiguration.includeComponentTokens) {
    allowedTopLevelGroups.push('components')
  }
  
  tokens = tokens.filter(token => {
    const topLevelGroup = token.tokenPath && token.tokenPath.length > 0 ? token.tokenPath[0].toLowerCase() : ''
    return allowedTopLevelGroups.includes(topLevelGroup)
  })
