...
export function tokenVariableName(token: Token, tokenGroups: Array<TokenGroup>, collections: Array<DesignSystemCollection> = []): string {
  ...
  // Build the prefix array, conditionally including the token type prefix
  const prefixArray = [exportConfiguration.globalNamePrefix]
  if (exportConfiguration.includeTokenType) {
    prefixArray.push(prefix)
  }
  if (exportConfiguration.tokenNameStructure === TokenNameStructure.CollectionPathAndName && collection?.name) {
    prefixArray.push(collection.name)
  }
  ...
  return NamingHelper.codeSafeVariableNameForToken(
    ...
    prefixArray.filter(Boolean).join('-')
  )
}
